<template>
	<div class="account">
		...
		<div class="center">
			...
			<div class="pay">
				<p class="yingfu r">应付金额：<span>￥{{(total+freight_price)|keep2Num}}</span></p>
				<div class="r">
...
<script>
	...
	export default {
		...
		methods: {
			...
			save() {
				console.log(this.content_id, this.type_id, this.raised_id)
				console.log(this.content_name, this.type_name, this.raised_name);
				console.log(this.invoiceTit)
			...
			placeOrder() {
				...
				this.HTTP(this.$httpConfig.orderBeginFromPanicGood, {
					invoice_id: this.invoiceGroup,
					address_id: this.addressInfo.id,
					remarks: this.remarks,
          goods_id: this.$route.query.id,
          goods_num: this.$route.query.num
				}, 'post').then((res) => {
					this.$router.push({
						path: '/payment',
						query: {
			...
			toDetails() {
				window.open(window.location.origin + '/shopsn_product/' + this.goodsData.id);
			},
			toStore() {
