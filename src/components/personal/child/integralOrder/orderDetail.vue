<template>
  <div class="myOrder l">
    <div class="dingDan">
      ...
      <div class="zhuangtai">
        <p>当前订单状态：{{Status[goods.status]}}</p>
      ...
      <div class="Maijai">
        ...
        <ul>
          ...
          <li class="l">支付宝交易号：{{order.order_sn}}</li>
          <li class="l">创建时间：{{~~order.create_time | formatDate}}</li>
          <li class="l">发货时间：{{~~order.over_time | formatDate}}</li>
          <li class="l">付款时间：{{~~order.pay_time | formatDate}}</li>
          <li class="l">交货时间：{{~~order.delivery_time | formatDate}}</li>
        </ul>
      </div>
      <div class="shangpin">
        <div class="up">
        ...
        <div class="down">
          <p><img :src="URL + '/' + goods.pic_url" /><span>{{goods.title}}</span></p>
          <p>
            <span>暂无数据</span>
          </p>
          <p>{{Status[Number(goods.status)]}}</p>
          <p>￥{{goods.price_member}}</p>
          <p>{{goods.goods_num}}</p>
          <p>无</p>
          <p><span>￥{{goods.price_member * goods.goods_num}}</span><span>（含运费：￥0.00）</span></p>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    data() {
      return {
        ...
        address: [],
        order: [],
        Status:['立即付款','已支付','发货中','已发货','已收货','退货审核中','审核失败','审核成功','退款中','退款成功'],
      };
    },
    created() {
      console.log(this)
    ...
    openkefu(item) {
      ...
      Order() {
        this.HTTP(this.$httpConfig.orderDetailByIntegral, {
          ...
          this.goods = res.data.data.goods[0]
