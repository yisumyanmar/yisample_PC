            ...
            <div class="order-info l">
              <div class="zuo">
                <div class="huowu">
                  ...
                  <span v-if="checkOverTime(item.over_time)" @click="GoTo(item.id,goods.goods_id,item.order_sn_id,goods.package_id)" class="goods-operate" target="_blank" >售后处理</span>
                </div>
              </div>
            </div>
          </div>
...
<script>
 ...
 if (res.data.status == 10001) {
      ...
      Order() {
        this.HTTP(this.$httpConfig.package.packageToBeEvaluated, {
