<template>
	<div class="account">
		...
		<div class="logo">
			<img @click="toHome" src="../../assets/img/logo.jpg" />
			<p>结算页</p>
		</div>
		<div class="center">
			<ul class="buzhou">
			...
			<div class="xinxi">
				...
				<div class="bottom">
					...
					<div class="dianpu" v-for="(item, index) in buyData.store" :key="index">
						<p class="name">店铺：<span @click="toStore(item.id)">{{item.shop_name}}</span><img src="../../assets/img/qq.jpg" /></p>
						<ul class="order-list">
							<li class="order-info" v-for="(value, key) in buyData.goods" :key="key">
								...
								<li class="fapiao">发票信息：<span class="invoice-info" v-if="invoiceTit[item.id]">{{invoiceTit[item.id][0]}}</span><span v-if="invoiceTit[item.id]" class="invoice-info">{{invoiceTit[item.id][1]}}</span><span v-if="invoiceTit[item.id]" class="invoice-info">{{invoiceTit[item.id][2]}}</span>
							...
							<li class="order-item clearfix">
								...
								<div class="order-extInfo r">
									<div class="order-deliveryMethod">
										<span>运送方式：</span><span>普通运送 </span><span>快递 </span>
			...
			<div class="jiesuan">
				<p><span>{{total_num}} </span>件商品，商品总金额：￥{{total_price|keep2Num}}</p>
				<p>积分：-￥0.00</p>
				<p>优惠券：-￥0.00</p>
				<p>运费：￥{{freight_price|keep2Num}}</p>
			</div>
			<div class="pay">
				<p class="yingfu r">应付金额：<span>￥{{(total_price + freight_price)|keep2Num}}</span></p>
				<div class="r">
      ...
      this.getData();
		...
		methods: {
			...
			placeOrder() {
				...
				let total = parseFloat(this.buyData.total_price) +parseFloat(this.freight_price);
				this.HTTP(this.$httpConfig.partsBuyNow, {
			...
			getData(){
				this.HTTP(this.$httpConfig.goodsComboBuyNow, {
